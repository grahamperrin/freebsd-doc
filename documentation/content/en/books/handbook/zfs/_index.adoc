---
title: Chapter 21. The Z File System (ZFS)
...
tags: ["ZFS", "filesystem", "administration", "pool", "zpool", "features", "terminology", "RAID-Z"]
showBookMenu: true
weight: 25
path: "/books/handbook/"
aliases: ["/en/books/handbook/zfs-quickstart/","/en/books/handbook/zfs-zpool/","/en/books/handbook/zfs-zfs/","/en/books/handbook/zfs-zfs-allow/","/en/books/handbook/zfs-advanced/","/en/books/handbook/zfs-links/","/en/books/handbook/zfs-term/"]
---
...
The previous example created the `storage` pool.
...
Creating a pool requires forethought.

The most important decision is which types of vdevs to group the physical disks into. 
See the list of <<zfs-term-vdev,vdev types>> for details.

After creating a pool, most types of vdev do not allow addition of a disk.
Exceptions are: 

* mirrors, which do allow addition of a disk to the vdev
* stripes, which upgrade to mirrors by attaching a new disk to the vdev.

Although adding a new vdev expands a pool, the pool layout cannot change.
...
If the new disk is larger than the old disk, it may be possible to grow the pool, using the new space.
...
To manage the pool itself, use <<zfs-zpool,`zpool`>>.
...
<<zfs-zfs-allow,Delegate>>, <<zfs-zfs-send,replicate>>, <<zfs-zfs-snapshot,snapshot>>, <<zfs-zfs-jail,jail>> allows administering and destroying each dataset as a unit.
...
If there are child datasets, like <<zfs-term-snapshot,snapshots>> or other datasets, destroying the parent is impossible.
...
* Use the <<zfs-zfs-allow,ZFS Delegation>> system to allow a non-`root` user on each system to perform the respective send and receive operations.
...
Use <<zfs-term-quota,Dataset quotas>> to restrict the amount of space consumed by a particular dataset.
<<zfs-term-refquota,Reference Quotas>> work in much the same way, but count the space used by the dataset itself, excluding snapshots and child datasets.
Similarly, use <<zfs-term-userquota,user>> and <<zfs-term-groupquota,group>> quotas to prevent users or groups from using up all the space in the pool or dataset.
...
<<zfs-term-reservation,Reservations>> guarantee an always-available amount of space on a dataset.
...
Compression can also be a great alternative to <<zfs-zfs-deduplication,Deduplication>> because it does not require extra memory.
...
Enable deduplication in cases where savings are considerable and with enough available memory for the <<zfs-term-deduplication,DDT>>.
