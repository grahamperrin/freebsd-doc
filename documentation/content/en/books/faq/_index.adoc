...
FreeBSD 11.0-RELEASE included link:https://www.freebsd.org/releases/11.0R/relnotes/#storage-general[TRIM/UNMAP support on GELI-backed SSD storage providers]. 
See man:geli[8] option `-T`.

ZFS in FreeBSD stable/12: TRIM is enabled by default.
To disable TRIM, add this line to [.filename]#/etc/sysctl.conf#:
...
OpenZFS in FreeBSD 13.0-RELEASE and greater: see man:zpool-trim[8], and `autotrim` in man:zpoolprops[7].
