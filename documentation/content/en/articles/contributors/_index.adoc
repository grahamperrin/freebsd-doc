...
FreeBSD was https://freebsdfoundation.org/freebsd/timeline/[derived from 386BSD Release 0.1 by William and Lynne Jolitz], though almost none of the original 386BSD specific code remains.
It was essentially re-implemented from the 4.4BSD-Lite release provided by the Computer Science Research Group (CSRG) at the University of California, Berkeley and associated academic contributors.

Portions of NetBSD and OpenBSD have been integrated into FreeBSD, we therefore thank all contributors to NetBSD and OpenBSD.
